...
void WebConfig::Init(const char* appName, const char* defAPName, const char* defAPPass, bool doReset)
{
	...
	if (doReset || !LoadSettings())
	{
		...
		udpPort = 0;
		tcpPort = 0;
    strncpy(ntpServer, "time.nist.gov", 32);
...
void WebConfig::ProcessHTTP()
{
  ...
  s += "var ntpServer = document.getElementById('ntpServer').value;\r\n";
  s += "if (webPassword != webPassword2) { alert('WEB passwords dont match'); return; }\r\n";
	...
	s += "window.location.search=webPort + '&' + webLogin + '&' + webPassword + '&' + btoa(webLogin+':'+webPassword) + '&' + (isAP?'1':'0') + '&' + apName + '&' + apPassword + '&' + apChannel + '&' + ssid + '&' + password + '&' + udpPort + '&' + tcpPort + '&' + ntpServer + '&';\r\n";
	s += "}\r\n";
	s += "</script>\r\n";
  ...
  // misc configuration
  s += "<table border=1>\r\n";
  s += "<tr><td colspan=2 bgcolor=#E0E0E0><b>Misc</b></td></tr>\r\n";
  s += "<tr><td>NTP Server</td><td><input type=text id='ntpServer' value='" + String(ntpServer) + "'></td></tr>\r\n";
  s += "</table>\r\n";
...
void WebConfig::ProcessParms(String req)
{
  ...
  strncpy(ntpServer, Token(&pPos,"&"), 32);
